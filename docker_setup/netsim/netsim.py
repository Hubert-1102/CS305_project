...
def autogen_click():
    ...
    with open(click_conf, 'w') as click_conf_file:
        if args.topology == 'onelink':
            click_conf_file.write(
                ONELINK_TEMPLATE % (LINK_PORT[0], APACHE_SERVER_PORT[0], INITIAL_BANDWIDTH_BIT_PER_SEC, CONTROL_PORT))
        if args.topology == 'twolink':
            click_conf_file.write(
                TWOLINK_TEMPLATE % (LINK_PORT[0], APACHE_SERVER_PORT[0], INITIAL_BANDWIDTH_BIT_PER_SEC,
        ...
        if args.topology == 'sharelink':
            click_conf_file.write(SHARELINK_TEMPLATE % (LINK_PORT[0], APACHE_SERVER_PORT[0],
                                                        LINK_PORT[1], APACHE_SERVER_PORT[1],
                                                        INITIAL_BANDWIDTH_BIT_PER_SEC, CONTROL_PORT))
...
class SendFile(threading.Thread):
    ...
    def reply(self, address):
        self.socket.sendto(args.servers.encode(), address)


if __name__ == "__main__":
    # set up command line args
    parser = argparse.ArgumentParser(description='Launch bandwidth limited servers or multiple servers')
    parser.add_argument('topology', choices=['onelink', 'twolink', 'sharelink', 'servers'],
                        help='topology of bandwidth limited servers')
    parser.add_argument('command', choices=['start', 'stop', 'restart', 'run'],
                        help='start/stop/restart the servers, or run a series of link events?')
    parser.add_argument('-l', '--log', default=None,
                        help='log file for logging events (overwrites file if it already exists)')
    parser.add_argument('-e', '--events', default=None,
                        help='specify events file to use in place of the one contained in the topology directory')
    parser.add_argument('-s', '--servers', default=None,
                        help='the file contains the port number of servers needed to be created in server mode')
    parser.add_argument('-q', '--quiet', action='store_true', default=False, help='only print errors')
    parser.add_argument('-v', '--verbose', action='store_true', default=False,
                        help='print debug info. --quiet wins if both are present')
